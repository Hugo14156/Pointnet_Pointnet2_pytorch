...
class VoyantDataset(Dataset):
    ...
    def __init__(
        ...
        csv_paths = sorted(os.listdir(data_root))
        if split == "train":
            csvs_split = []
        ...
        for csv in tqdm(csvs_split, total=len(csvs_split)):
            ...
            for i in range(len(csv)):
                ...
                if label == "SM_WarehouseLine_A1":
                    ...
                    labels.append("floor")
                elif label == "SM_YellowPaint_A1":
                    labels.append("stairs")
                elif label == "SM_WhitePaint_B1":
                    labels.append("beam")
                elif label == "SM_WhitePaint_A1":
                    labels.append("wall")
                elif label == "SM_Metal_A1":
                    labels.append("bolt")
                elif label == "SM_GlossyMetal_C1":
                    labels.append("walkway floor")
                elif label == "SM_BluePaint_A1":
                    labels.append("beam")
                elif label == "SM_GlossyMetal_B1":
                    labels.append("window frame")
                else:
            ...
            self.csv_points.append(points), self.csv_labels.append(labels)
            self.csv_coord_min.append(coord_min), self.csv_coord_max.append(coord_max)
        ...
        csv_idxs = []
        for index in range(len(csvs_split)):
            csv_idxs.extend([index] * int(round(sample_prob[index] * num_iter)))
        self.csv_idxs = np.array(csv_idxs)
        print("Totally {} samples in {} set.".format(len(self.csv_idxs), split))
